              ...
              {cartItems.length === 0 ? (
                ...
                cartItems.map(item => {
                  ...
                  return (
                    <div
                      key={item.item_id}
                      className={`flex gap-4 p-4 border-b border-stone-200 transition-all duration-300 ${removingId === item.item_id ? 'opacity-0 translate-x-10 h-0 p-0 overflow-hidden' : ''
                        }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {ratti && <h4 className="text-sm font-semibold text-stone-900 truncate mb-1">{`Ratti : ${ratti}`}</h4>}
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-base font-bold text-stone-900">₹{total.toLocaleString()}</span>
                        </div>
